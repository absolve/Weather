...
public class HttpUtil {
    ...
    //和风天气地址
     final static String hfWeatherhost = BuildConfig.HFWeatherhost;
    ...
    /**
     * 获取天气预报  openweather天气
     *
     * @param cityname 城市名字
     * @param lang     语言 zh_cn  en
     * @param callback 回调函数
     */
    public static void getForecastOW(String cityname, String lang, Callback callback) {
        if (client == null) {
            return;
        }
        HttpUrl.Builder urlBuilder = HttpUrl.parse(Weatherhost).newBuilder();
        urlBuilder.addPathSegment("forecast");
        urlBuilder.addPathSegment("daily");
        urlBuilder.addQueryParameter("q", cityname);
        urlBuilder.addQueryParameter("lang", lang);
        urlBuilder.addQueryParameter("appid", BuildConfig.owkey);
        Request request = new Request.Builder()
     ...
     * @param callback 回调函数
     */
    public static void getForecastOW(String lat, String lon, String lang, Callback callback) {
        if (client == null) {
            return;
        }
        HttpUrl.Builder urlBuilder = HttpUrl.parse(Weatherhost).newBuilder();
